...
Bus schedule YAML files are generated by `scripts/update_schedules.py`, which fetches from two sources: the bustago.or.kr API (ICN routes) and the txbus.t-money.co.kr operation-info endpoint (CJJ routes). Each file contains:

- **ICN routes** (`icn-govcomplex.yml`, `icn2-govcomplex.yml`, `icn-doryong.yml`, `icn2-doryong.yml`, and the return direction `govcomplex-icn.yml`, `govcomplex-icn2.yml`, `doryong-icn.yml`, `doryong-icn2.yml`): `updated`, `first_bus`, `last_bus`, `rows` (list of `time`/`class`/`fare`) — from **bustago**
- **CJJ routes** (`cjj-yuseong.yml`, `yuseong-cjj.yml`): `updated`, `first_bus`, `last_bus`, `times` (list of departure times) — from **txbus**
...
The CJJ ↔ Yuseong route uses **txbus** (the TmoneyGO backend), not bustago: bustago omits several 서울고속/새서울고속 우등 departures on that route (e.g. 06:30, 09:20, 12:10, 15:20, 18:50) that the airport kiosk and TmoneyGO do show, so txbus is used as the complete source. txbus terminal codes (청주공항 `2814201`, 유성복합 `3417501`) come from its `/otck/readTrmlList.do` and are unrelated to bustago IDs. The ICN airport routes are bustago-exclusive (txbus does not carry them), so they stay on bustago. The txbus booking endpoint rejects scripted requests, so the script uses the open operation-info endpoint (`/runinf/readRunInfList.do`), which returns an HTML page (parsed with a regex) rather than JSON.
